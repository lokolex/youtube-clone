...
import { ISearchQueryRes } from '../../../../types/custom_types';
...
const Search = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<ISearchQueryRes | undefined>(
  ...
  return (
    <div className="mt-[-64px] px-3">
      <h3 className="scroll-m-20 my-5 text-2xl font-semibold tracking-tight">Search Results</h3>
          ...
          <div className="col-span-12 md:col-span-8">
            ...
            <div className="my-6 flex items-center space-x-3">
              <Avatar className="w-11 h-11">
                <AvatarImage src={item.snippet.thumbnails.medium.url} alt="AlexStr" />
                <AvatarFallback>AS</AvatarFallback>
              </Avatar>

              <p className="text-sm dark:text-background-light">{item.snippet.channelTitle}</p>
            </div>

            <p className="text-sm dark:text-background-light">{item.snippet.description}</p>
          </div>
        </Link>
      ))}
    </div>
