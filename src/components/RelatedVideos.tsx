...
import type { Video } from '../../types/custom_types';

const RelatedVideos = ({ video }: { video: Video }) => {
  return (
    <Link href={`/watch/${video?.id}`} className="h-32 my-4 flex gap-3 justify-between">
      ...
      <div className="flex-1">
        ...
        <div className="text-xs flex items-center text-background-light">
          {formatCount(Number(video.viewCount))} views {<Dot />}{' '}
