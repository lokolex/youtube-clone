...
const Thumbnail = ({ video }: { video: Video }) => {
  return (
    <Link href={`/watch/${video.id}`} className="w-[320px] mx-auto md:w-[350px] my-4">
      ...
      <div className="flex space-x-2 py-3">
        <Avatar>
          <AvatarImage src={video.channel.channelImage} alt={video.channel.channelTitle} />
        ...
        <div className="flex flex-col">
          ...
          <div className="flex text-sm dark:text-background-light text-background-dark">
            <p>{formatCount(Number(video.viewCount))}</p> <Dot />{' '}
